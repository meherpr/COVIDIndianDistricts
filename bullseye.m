% Daniel Ennis (2020). 
% Bullseye Polar Data Plot (https://www.mathworks.com/matlabcentral/fileexchange/16458-bullseye-polar-data-plot), 
% MATLAB Central File Exchange. Retrieved September 16, 2020.
...
function [h,t]=bullseye(data,Labels,varargin);
...
%%%%%% Modification Brunno
LabelsX = X(4,size(data,2):size(data,2):end);
LabelsY = Y(4,size(data,2):size(data,2):end);
for j=1:size(data,1)
...
ang = rad2deg(ang(1));
if abs(ang)>=rad2deg(pi/2) && abs(ang)<=rad2deg(3*pi/2)
ang = ang - rad2deg(pi);
%meher text(LabelsX(j),LabelsY(j),['ROI ',num2str(j)],'FontSize',12,...
%'Color',[0 0 0],'HorizontalAlignment','right','Rotation',ang)
text(LabelsX(j),LabelsY(j),Labels{j},'FontSize',12,...
'Color',[0 0 0],'HorizontalAlignment','right','Rotation',ang)

else
% meher text(LabelsX(j),LabelsY(j),['ROI ',num2str(j)],'FontSize',12,...
%'Color',[0 0 0],'Rotation',ang)
text(LabelsX(j),LabelsY(j),Labels{j},'FontSize',12,...
'Color',[0 0 0],'Rotation',ang)
